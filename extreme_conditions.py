...
# # # # Saving Function
# # # # -----------------------
# # # def save_results_to_folder(posterior_r, posterior_mu, posterior_Q, alerts, folder_name="extremeconditions"):
...
# # #     # Define different noisy observation scenarios for various conditions
# # #     Q_obs_extreme_blockage = simulate_noisy_observations(Q_true * 0.05)  # Extreme Blockage
# # #     Q_obs_blockage = simulate_noisy_observations(Q_true * 0.3)          # Blockage
# # #     Q_obs_reduced_flow = simulate_noisy_observations(Q_true * 0.7)      # Reduced Flow
# # #     Q_obs_near_normal_flow = simulate_noisy_observations(Q_true * 1.1)  # Near Normal Flow
...
#     # Plot Posterior for Radius
#     axes[0].hist(posterior_r, bins=30, density=True, color='skyblue', alpha=0.7, label="Posterior Radius")
#     axes[0].set_title("Posterior of Radius (r)")
#     axes[0].set_xlabel("Radius (m)")
#     axes[0].set_ylabel("Density")
    
#     # Plot Posterior for Viscosity
#     axes[1].hist(posterior_mu, bins=30, density=True, color='lightgreen', alpha=0.7, label="Posterior Viscosity")
#     axes[1].set_title("Posterior of Viscosity (mu)")
#     axes[1].set_xlabel("Viscosity (Pa·s)")
#     axes[1].set_ylabel("Density")

#     # Plot Posterior for Flow Rate
#     axes[2].hist(posterior_Q, bins=30, density=True, color='lightcoral', alpha=0.7, label="Posterior Flow Rate")
#     axes[2].set_title("Posterior of Flow Rate (Q)")
#     axes[2].set_xlabel("Flow Rate (m³/s)")
#     axes[2].set_ylabel("Density")
...
#     # Define different noisy observation scenarios for various conditions
#     Q_obs_extreme_blockage = simulate_noisy_observations(Q_true * 0.05, noise_percent=0.1)  # Extreme Blockage
#     Q_obs_blockage = simulate_noisy_observations(Q_true * 0.3, noise_percent=0.15)         # Blockage
#     Q_obs_reduced_flow = simulate_noisy_observations(Q_true * 0.7, noise_percent=0.2)      # Reduced Flow
#     Q_obs_near_normal_flow = simulate_noisy_observations(Q_true * 1.1, noise_percent=0.1)  # Near Normal Flow
...
#     # Now create the plots with legends
#     radius_values = np.linspace(0.01, 1.0, 100)  # Example radius range for flow calculation
#     flow_values = poiseuille_flow_vec(radius_values, true_mu)

#     # Plot the results
#     plt.figure(figsize=(12, 8))

#     # Plot the observed data for each case with legends
#     plt.plot(radius_values, Q_obs_extreme_blockage, label="Extreme Blockage", color='red')
#     plt.plot(radius_values, Q_obs_blockage, label="Blockage", color='orange')
#     plt.plot(radius_values, Q_obs_reduced_flow, label="Reduced Flow", color='blue')
#     plt.plot(radius_values, Q_obs_near_normal_flow, label="Near Normal Flow", color='green')

#     # Adding mu±sigma markers
#     mu = np.mean(Q_true)  # Assuming mean flow is the true value
#     sigma = np.std(Q_true)  # Assuming standard deviation for noise

#     # Adding vertical lines for different levels of sigma
#     plt.axvline(mu + sigma, color='purple', linestyle='--', label="mu + sigma")
#     plt.axvline(mu + 2 * sigma, color='pink', linestyle='--', label="mu + 2sigma")
#     plt.axvline(mu + 3 * sigma, color='gray', linestyle='--', label="mu + 3sigma")
#     plt.axvline(mu - sigma, color='purple', linestyle='--', label="mu - sigma")
#     plt.axvline(mu - 2 * sigma, color='pink', linestyle='--', label="mu - 2sigma")
#     plt.axvline(mu - 3 * sigma, color='gray', linestyle='--', label="mu - 3sigma")
...
#     plt.title("Flow vs. Radius for Different Blockage Conditions")
#     plt.legend()
